...
TOKEN_SYMBOL: immutable(String[16]) 
...
EIP712_DOMAIN_NAMEHASH: immutable(bytes32) 
...
@external
def __init__(_name: String[64], _symbol: String[16], baseURI: String[96], artist: address ):
    """
    ...
    EIP712_DOMAIN_NAMEHASH = keccak256(TOKEN_NAME)
...
@external
def updateBaseURI(baseURI: String[96]):
...
def _transferFrom(owner: address, receiver: address, tokenId: uint256, sender: address):
    ...
    """
    # Check requirements
    assert self._isApprovedOrOwner(sender, tokenId), "Error-Not approved or owner!"
    assert receiver != ZERO_ADDRESS, "Error-Send to zero address but not a burn!"
    assert owner != ZERO_ADDRESS, "Error-Send from zero address but not a mint!"
    assert self.ownerOf[tokenId] == owner, "Error-From address is not current owner!"
...
def mint(receiver: address) -> bool:
    ...
    """
    # Throws if `msg.sender` is not the minter
    assert msg.sender == self.minter, "Error-must be minter to mint!"

    # Throws if `receiver` is zero address
    assert receiver != ZERO_ADDRESS, "Error-can't mint to ZERO_ADDRESS!"

    # Throws if we've minted the whole supply
    assert self.totalSupply < MAX_TOTAL_SUPPLY, "Error-minting exhausts max token supply!"
