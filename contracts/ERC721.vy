...
@external
def __init__(_name: String[64], _symbol: String[16], _baseURI: String[128], _minter: address):
    """
    ...
    self.name = _name
    self.symbol = _symbol
    # _baseURI example : "https://opensea-creatures-api.herokuapp.com/api/creature/"
...
def _uint_to_string(_value: uint256) -> String[78]:
    ...
    for i in range(78):
        ...
        buffer = raw_call(
            ...
            IDENTITY_PRECOMPILE, # returns the input for efficient data copying
            concat(buffer, char),
            max_outsize=78,
            is_static_call=True
        )
...
@view
@external
def tokenURI(_tokenId: uint256) -> String[256+78]:
    # Check if tokenId is valid
    assert _tokenId <= self.totalSupply
    _uri: String[256+78] = concat(self.baseURI, self._uint_to_string(_tokenId))
    return _uri 
