...
@pytest.fixture
def token_contract(ERC721, accounts):
...
def test_supportsInterface(token_contract):
    ...
    assert token_contract.supportsInterface(ERC721_INTERFACE_ID) == 1
...
def test_balanceOf(token_contract, accounts):
    someone = accounts[1]
    assert token_contract.balanceOf(someone) == 3
...
def test_getApproved(token_contract, accounts):
    ...
    # assert token_contract.getApproved(SOMEONE_TOKEN_IDS[0]) is None

    token_contract.approve(operator, SOMEONE_TOKEN_IDS[0], {"from": someone})

    assert token_contract.getApproved(SOMEONE_TOKEN_IDS[0]) == operator
...
def test_transferFrom_by_owner(token_contract, accounts):
    ...
    tx_hash = token_contract.transferFrom(
        someone, operator, SOMEONE_TOKEN_IDS[0], {"from": someone}
    )

    assert token_contract.ownerOf(SOMEONE_TOKEN_IDS[0]) == operator
    assert token_contract.balanceOf(someone) == 2
...
def test_transferFrom_by_approved(token_contract, accounts):
    ...
    tx_hash = token_contract.transferFrom(
        someone, operator, SOMEONE_TOKEN_IDS[1], {"from": operator}
    )

    assert token_contract.ownerOf(SOMEONE_TOKEN_IDS[1]) == operator
    assert token_contract.balanceOf(someone) == 2
    assert token_contract.balanceOf(operator) == 2
...
def test_transferFrom_by_operator(token_contract, accounts):
    ...
    token_contract.setApprovalForAll(operator, True, {"from": someone})
    tx_hash = token_contract.transferFrom(
        someone, operator, SOMEONE_TOKEN_IDS[2], {"from": operator}
    )
...
def test_safeTransferFrom_by_owner(token_contract, accounts):
    ...
    # transfer by owner
    tx_hash = token_contract.safeTransferFrom(
        someone, operator, SOMEONE_TOKEN_IDS[0], b"101", {"from": someone}
    )
